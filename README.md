...
O NBoleto é uma biblioteca para gerar boleto bancário **não registrado** (a forma mais simples e mais utilizada para web).
...
O NBoleto não imprime o boleto, não gera o HTML nem a imagem do boleto, apenas calcula e gera as informações: 

- Nosso Número (com dígito verificador)
- Linha digitavel 
- Código de Barras (imagem em Base64)

A aplicação que usa o NBoleto a responsavel por criar o design do boleto e usar a classe `Boleto` para preencher todos os campos.
Desta forma, a aplicação fica livre para implementar como precisa.
